...
$countResult = $database->pdo->query("SELECT COUNT(*) FROM migrations");
$migrationsCount = (int) $countResult->fetch(PDO::FETCH_NUM)[0];

if ($migrationsCount === 0) {
    $database->pdo->query("INSERT INTO migrations (version) VALUES (-1);");
}

$maxver = (int) $database->pdo->query("SELECT MAX(version) FROM migrations")->fetch(PDO::FETCH_NUM)[0];
...
foreach ($migrations as $migration) {
    ...
    $stmt = $database->pdo->prepare("INSERT INTO migrations (version) VALUES (?)");
